 ...
 */
var HashRing = require('fixed-position-hashring')
...
Client.config = {
  ...
  , retryFactor: 3          // exponential factor to use when retrying
  , randomizeRetries: true  // randomize the time between retries
  , retry: 30000            // timeout between retries, all call will be marked as cache miss
  , maxRetry: 60000         // retries can be exponential, this is the maximum we wait between retries
...
(function (nMemcached) {
  ...
  memcached.connect = function connect(server, callback) {
    ...
    manager.factor = memcached.retryFactor;
    manager.minTimeout = memcached.retry;
    manager.maxTimeout = memcached.maxRetry;
    manager.randomize = memcached.randomizeRetries;
    ...
    (function(serverTokens) {
      manager.on('error', function err(e) {
        ...
        memcached.connectionIssue(e, {
          serverAddress: server,
          tokens: [].concat(serverTokens)
        });
      });
    })(serverTokens);
  ...
  memcached.command = function memcachedCommand(queryCompiler, server) {
    ...
    this.connect(server, function allocateMemcachedConnection(error, S) {
      ...
      // successful connection, remove any pending issues
      delete memcached.issues[server];
    ...
    if (server in this.issues) {
      ...
      Utils.fuse(issues, {
        ...
        , reconnected: function reconnected(details) {
            ...
            memcached.HashRing.addServer(server);
          }
